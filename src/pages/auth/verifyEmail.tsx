...
type Props = React.ReactChild & { verifyEmail: Function, history: Function, location: Object };

// const VerifyEmail: React.FunctionComponent<Props> = (props) => {
const VerifyEmail: any = (props: any): any => {

  const { history, verifyEmail, location : { search } } = props;
	...
	return (
		<section className="verifyEmail">
			...
			<div className="formWrapper">
				<button onClick={makeVerifyEmail}>
          Verify Email
        </button>
			</div>
...
const VerifyEmailRouteComponent: any = connect(
  ...
  {
    verifyEmail: (token: string, history: any): any => ({ type: userActions.verifyEmail.request, payload: { token }, history }),
  }
)(VerifyEmail);
